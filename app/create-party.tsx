...
export default function CreatePartyScreen() {
  ...
  const AddRule = () => {
    setRules((prev) => {
      ...
      return [
        ...prev,
        { id: nextId, color: nextColor, name: "", description: "", points: 10 },
      ];
    });
  };
  ...
  const handleCreateParty = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Erreur",
        "Impossible de créer la soirée. Vérifiez votre connexion.",
      );
    }
  };
        ...
        <View style={styles.rulesContainer}>
          {rules.map((rule) => (
            <RuleRow
              key={rule.id}
              rule={rule}
              onUpdate={(field: keyof Rule, val: string | number) =>
...
const RuleRow = ({
  ...
  rule: Rule;
  onUpdate: (field: keyof Rule, val: string | number) => void;
}) => (
  <View style={styles.ruleWrapper}>
    ...
    <View style={[styles.ruleCard, { backgroundColor: rule.color }]}>
      ...
      <View style={styles.pointsColumn}>
        <TouchableOpacity
          onPress={() => onUpdate("points", Math.max(0, rule.points - 5))}
        >
